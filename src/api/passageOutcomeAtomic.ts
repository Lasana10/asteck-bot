...
const ALLOWED_OUTCOMES = new Set(['successful_pickup','road_inaccessible','meeting_point_incorrect','passenger_no_show','driver_cancelled','passenger_cancelled']);
...
async function authoritativeProfile(subjectId: string): Promise<Identity | null> {
  const { data: profile, error } = await supabase.from('profiles').select('id, role, is_active, approval_status, operator_application_status').eq('id', subjectId).maybeSingle();
...
async function ingestEmpiricalSpeedEvidence(outcomeId: string) {
  ...
  if (error) {
    console.error('AFAT speed evidence ingest failed after committed passage outcome:', error);
    return { status: 'unavailable', reason: 'speed_evidence_ingest_failed' };
  }
  return data || { status: 'unavailable', reason: 'speed_evidence_ingest_empty' };
}
...
router.post('/passages/intents/:id/outcome', async (req: Request, res: Response) => {
  try {
    ...
    if (!ALLOWED_OUTCOMES.has(outcomeType)) return res.status(400).json({ error: 'Unsupported passage outcome.' });
    const key = idempotencyKey(req);
    if (key.length < 8 || key.length > 200) return res.status(400).json({ error: 'A stable Idempotency-Key of 8-200 characters is required for passage outcomes.' });

    const { data: passage, error: passageError } = await supabase.from('passage_intents').select('id, passenger_id, operator_id, status').eq('id', req.params.id).maybeSingle();
    ...
    if (!privileged && !passengerOwns && !operatorOwns) return res.status(403).json({ error: 'Passage access denied.' });
    if (['completed', 'cancelled'].includes(String(passage.status || ''))) return res.status(409).json({ error: 'A terminal passage cannot receive a new pickup outcome.' });
    if (outcomeType === 'passenger_no_show' && !operatorOwns && !privileged) return res.status(403).json({ error: 'Only the assigned operator can report a passenger no-show.' });
    if (outcomeType === 'driver_cancelled' && !operatorOwns && !privileged) return res.status(403).json({ error: 'Only the assigned operator can report a driver cancellation.' });
    if (outcomeType === 'passenger_cancelled' && !passengerOwns && !privileged) return res.status(403).json({ error: 'Only the passenger can report a passenger cancellation.' });
    ...
    const { data, error } = await supabase.rpc('afat_record_passage_outcome', {
      p_passage_intent_id: passage.id, p_reporter_id: identity.id, p_outcome_type: outcomeType,
      p_responsibility: responsibility, p_notes: notes, p_evidence: evidence,
      p_expected_status: String(passage.status || ''), p_idempotency_key: key,
    });
    if (error) {
      const code = String(error.code || '');
      const message = String(error.message || 'Passage outcome could not be committed.');
      if (code === '40001' || code === '23514' || /status changed|terminal passage/i.test(message)) return res.status(409).json({ error: message });
