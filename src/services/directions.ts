...
export class DirectionsService {
  ...
  /**
   * TRAFFIC CROSS-CHECK: Query Google Maps for real-time traffic conditions
   * at a specific location. Used by VerifierAgent to silently confirm incidents.
   * 
   * Creates a small trip (origin → offset point ~500m away) and compares
   * duration_in_traffic vs. normal duration.
   * 
   * Returns: 'congested' | 'slow' | 'normal' | 'unknown'
   */
  static async getTrafficCondition(location: Coordinates): Promise<'congested' | 'slow' | 'normal' | 'unknown'> {
    if (!GOOGLE_MAPS_KEY) {
      return 'unknown'; // Graceful fallback — no key, no cross-check
    }

    try {
      // Create a small probe: check travel time from the incident point to a point ~500m north
      const probeDestination = {
        latitude: location.latitude + 0.005,  // ~500m north
      ...
      const response = await axios.get(
        'https://maps.googleapis.com/maps/api/distancematrix/json',
        {
          params: {
            ...
            destinations: `${probeDestination.latitude},${probeDestination.longitude}`,
            mode: 'driving',
      ...
      if (!element || element.status !== 'OK') return 'unknown';

      const normalDuration = element.duration?.value || 0;       // seconds (typical)
      const trafficDuration = element.duration_in_traffic?.value || 0;  // seconds (live)

      if (normalDuration === 0 || trafficDuration === 0) return 'unknown';

      const ratio = trafficDuration / normalDuration;

      console.log(`[TrafficProbe] ${location.latitude.toFixed(4)},${location.longitude.toFixed(4)} → ratio: ${ratio.toFixed(2)} (normal: ${normalDuration}s, live: ${trafficDuration}s)`);

      if (ratio >= 1.8) return 'congested';  // 80%+ slower than normal
      if (ratio >= 1.4) return 'slow';       // 40%+ slower than normal
      return 'normal';
    } catch (error) {
      console.warn('[TrafficProbe] Google Maps cross-check failed:', error);
      return 'unknown';
    }
  }
}
