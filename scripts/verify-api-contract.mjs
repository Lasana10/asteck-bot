...
const files = {
  ...
  adaptiveHome: readFileSync(join(root, 'dashboard', 'src', 'components', 'shared', 'AdaptiveRoleHome.tsx'), 'utf8'),
  interactiveMap: readFileSync(join(root, 'dashboard', 'src', 'components', 'shared', 'InteractiveMap.tsx'), 'utf8'),
  workspacePrimitives: readFileSync(join(root, 'dashboard', 'src', 'components', 'workspaces', 'WorkspacePrimitives.tsx'), 'utf8'),
...
const checks = [
  ...
  ['mobility map: sanitized feed exposes trusted Atlas geography', files.routes, 'atlas_nodes: scopedAtlasNodes'],
  ['admin live map: trusted Atlas geography is published alongside live overlays', privilegedRoute("router.get('/ops/live-map'", "router.get('/ops/report-center'"), 'atlas_nodes: scopedAtlasNodes.map'],
  ['frontend workspace feed hydrates Atlas geography', files.adaptiveHome, 'atlasNodes: mapResult.data.atlas_nodes || []'],
  ['frontend map receives Atlas geography explicitly', files.workspacePrimitives, 'atlasNodes={live.atlasNodes}'],
  ['MapLibre renders Atlas nodes as a GeoJSON layer', files.interactiveMap, "map.addSource('afat-atlas-nodes'"],
  ['mobility map publishes nearby Atlas edge geometry', files.routes, 'atlas_edges: scopedAtlasEdges'],
  ['frontend workspace feed hydrates Atlas edge geometry', files.adaptiveHome, 'atlasEdges: mapResult.data.atlas_edges || []'],
  ['frontend map receives Atlas edge geometry explicitly', files.workspacePrimitives, 'atlasEdges={live.atlasEdges}'],
  ['MapLibre renders Atlas road geometry as a GeoJSON layer', files.interactiveMap, "map.addSource('afat-atlas-edges'"],
  ['Atlas road styling preserves evidence maturity', files.interactiveMap, "['get', 'evidence_status']"],
