-- Mirror the verified live AFAT canonical router exactly in repository history.
-- Provisional OSM base geometry is routable while evidence confidence remains explicit.
...
begin
  ...
  where n.status='active'
    ...
    and exists (
      ...
      where e.status='active' and e.evidence_status in ('provisional','corroborated','verified')
...
comment on function public.afat_route_canonical(double precision,double precision,double precision,double precision,text,integer)
is 'AFAT canonical pgRouting router. Routes provisional/corroborated/verified geometry; ETA remains withheld until trusted speed evidence exists.';
