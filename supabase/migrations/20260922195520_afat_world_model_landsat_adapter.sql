...
set adapter_key='afat-landsat-scene-discovery',
    plan_state=case when p.plan_state='disabled' then p.plan_state else 'ready' end,
    updated_at=now()
from public.afat_city_profiles c
where p.city_profile_id=c.id and c.status='active' and p.source_key='landsat';
...
begin
  ...
  select v_city.id,s.source_key,
    case
      when s.source_key='openstreetmap' then 'afat-osm-city-ingest'
    ...
    case
      when s.source_key in ('afat_internal','openstreetmap','copernicus_sentinel','landsat') then 'ready'
      when cp.data_mode in ('reference_only','review_required') then 'reference_only'
      else 'needs_bulk_extract' end,
    case when s.source_key='afat_internal' then 100 when s.source_key='openstreetmap' then 95 when s.source_key='overture_maps' then 90 when s.source_key='copernicus_sentinel' then 85 when s.source_key='landsat' then 82 when s.source_key in ('google_open_buildings','microsoft_global_buildings') then 78 else 70 end,
    cp.feature_classes,
