-- Generic service-only candidate ingestion for licensed geospatial evidence sources.
-- Source registration + batch identity are required; this function never promotes trust.

create or replace function public.afat_ingest_geo_candidate_chunk(
  p_source_key text,
  p_batch_id uuid,
  p_records jsonb
)
...
begin
  select b.dataset_version into v_dataset_version
  from public.afat_geo_import_batches b
  where b.id=p_batch_id and b.source_key=p_source_key;

  if v_dataset_version is null then
    raise exception using errcode='22023',message='valid source import batch required';
  end if;
  ...
  for r in select value from jsonb_array_elements(p_records) loop
    ...
    v_point:=case when public.geometrytype(v_geom)='POINT' then v_geom else public.st_centroid(v_geom) end;

    select exists(
      select 1 from public.afat_geo_source_records s
      where s.source_key=p_source_key and s.external_feature_id=r->>'external_feature_id'
    ) into v_exists;
    ...
    insert into public.afat_geo_source_records(
      ...
      p_source_key,r->>'external_feature_id',p_batch_id,p_batch_id,coalesce(r->>'dataset_version',v_dataset_version),
      ...
      coalesce(r->'source_properties','{}'::jsonb),r->>'record_fingerprint','candidate',
      r->>'source_feature_kind',v_geom::public.geography,
...
revoke all on function public.afat_ingest_geo_candidate_chunk(text,uuid,jsonb) from public,anon,authenticated;
grant execute on function public.afat_ingest_geo_candidate_chunk(text,uuid,jsonb) to service_role;
