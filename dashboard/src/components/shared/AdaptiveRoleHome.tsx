...
export function AdaptiveRoleHome({ role, profile, membership, activeTab = 'home', onNavigate, onSignOut }: Props) {
  ...
  const tabs = activeTab === 'home' ? null
    : role === 'commuter'
      ? <PassengerWorkspaceTabs activeTab={activeTab} profile={profile} live={live} operations={operations} onSignOut={onSignOut} />
      : role === 'operator'
        ? <OperatorWorkspaceTabs activeTab={activeTab} profile={profile} live={live} missions={missions} operations={operations} onSignOut={onSignOut} />
        : role === 'planner'
          ? <PlannerWorkspaceTabs activeTab={activeTab} profile={profile} live={live} operations={operations} onSignOut={onSignOut} />
          : <InstitutionalWorkspaceTabs role={role as 'organization' | 'government' | 'admin'} activeTab={activeTab} profile={profile} membership={membership} live={live} operations={operations} onSignOut={onSignOut} />;
