...
export function MappingEvidenceReview(){
 ...
 return <section className="rounded-[1.5rem] border border-white/10 bg-slate-950/70 p-5">
  <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-widest text-cyan-200">Field evidence review</p><h2 className="mt-1 text-xl font-black">Human review before map promotion</h2></div><button onClick={load} disabled={busy} className="rounded-xl border border-white/10 bg-white/5 p-3"><RefreshCw className={`h-4 w-4 ${busy?'animate-spin':''}`}/></button></div>
