...
export function LiveOperationsControl(){
 ...
 return <section className="space-y-5">
   <div className="rounded-[1.6rem] border border-violet-300/15 bg-gradient-to-br from-violet-500/[0.09] to-slate-950/80 p-5">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-violet-300">{t('atlas.liveOps')}</p><h2 className="mt-2 text-2xl font-black">{t('atlas.liveOpsTitle')}</h2><p className="mt-2 max-w-3xl text-xs leading-5 text-white/45">This board reads the live operating state. It does not manufacture supply, demand or trips.</p></div><button onClick={load} disabled={busy} className="flex min-h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 text-[9px] font-black uppercase"><RefreshCw className={`h-4 w-4 ${busy?'animate-spin':''}`}/>Refresh</button></div>
