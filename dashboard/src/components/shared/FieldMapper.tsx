...
export function FieldMapper({cityKey='cm-yaounde',defaultMode='walk'}:{cityKey?:string;defaultMode?:string}){
 ...
 return <section className="rounded-[1.5rem] border border-emerald-300/15 bg-emerald-400/[0.04] p-5">
   ...
   <label className="mt-4 flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-xl border border-white/10 bg-black/20 px-3 text-[9px] font-black uppercase text-white/55"><Camera className="h-4 w-4"/>{photo?photo.name:'Add optional field photo'} · {pending} queued<input type="file" accept="image/jpeg,image/png,image/webp,image/heic" capture="environment" className="hidden" onChange={e=>setPhoto(e.target.files?.[0]||null)}/></label>
