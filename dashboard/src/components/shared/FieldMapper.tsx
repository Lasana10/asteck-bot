...
export function FieldMapper({cityKey='cm-yaounde',defaultMode='walk'}:{cityKey?:string;defaultMode?:string}){
 ...
 return <section className="rounded-[1.5rem] border border-emerald-300/15 bg-emerald-400/[0.04] p-5">
   <div className="flex items-start gap-3"><div className="rounded-xl bg-emerald-300/10 p-2"><MapPinPlus className="h-5 w-5 text-emerald-200"/></div><div><p className="text-[10px] font-black uppercase tracking-widest text-emerald-200">{t('atlas.fieldMapper')}</p><h2 className="mt-1 text-lg font-black">{t('atlas.fieldMapperTitle')}</h2><p className="mt-1 text-xs leading-5 text-white/45">Entrances, informal stops, pickup points, local names, missing paths, road surface and mode access are recorded as evidence first.</p></div></div>
