...
export function AtlasContributionPanel({defaultMode='walk'}:{defaultMode?:AtlasContributionMode}){
  ...
  return <section className="rounded-[1.5rem] border border-cyan-300/15 bg-gradient-to-br from-cyan-400/[0.08] to-slate-950/70 p-5 shadow-xl backdrop-blur-xl">
    <div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-cyan-200/75">{t('atlas.title')}</p><h2 className="mt-2 text-xl font-black">Improve AFAT while you move</h2><p className="mt-2 max-w-2xl text-xs leading-5 text-white/50">You do not need an AFAT booking. With your permission, normal movement can help AFAT learn which roads really work. Uncertain traces stay evidence until corroborated.</p></div><MapPinned className="h-6 w-6 shrink-0 text-cyan-200"/></div>
