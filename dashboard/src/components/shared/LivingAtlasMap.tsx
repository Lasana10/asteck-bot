...
export function LivingAtlasMap({cityKey='cm-yaounde'}:{cityKey?:string}){
    ...
    const render=()=>{
      ...
      if(layers.transit){
        ...
        for(const line of (transit.lines||[])){
          ...
          for(const link of (line.nodes||[])){
            const key=String(link.direction||'forward');
            if(!grouped.has(key))grouped.set(key,[]);
            grouped.get(key)!.push(link);
          }
          for(const [direction,links] of grouped.entries()){
            const coords=links.sort((a:any,b:any)=>Number(a.stop_sequence||0)-Number(b.stop_sequence||0)).map((link:any)=>{
              const node=nodeById.get(String(link.node_id));return node?[Number(node.longitude),Number(node.latitude)]:null;
            }).filter(Boolean);
            if(coords.length>=2) lineFeatures.push({type:'Feature',properties:{id:line.id,name:line.name||line.line_ref||'Transit line',mode:line.mode||'transit',direction,evidence_status:line.evidence_status||'limited',confidence:Number(line.confidence||0)},geometry:{type:'LineString',coordinates:coords}});
          }
        }
        if(lineFeatures.length){
          map.addSource('afat-transit-lines',{type:'geojson',data:{type:'FeatureCollection',features:lineFeatures} as any});
          map.addLayer({id:'afat-transit-lines',type:'line',source:'afat-transit-lines',paint:{'line-color':'#22d3ee','line-width':['interpolate',['linear'],['zoom'],10,2,15,5] as any,'line-opacity':0.82,'line-dasharray':[1.5,1] as any}});
        }
        ...
        if(transitNodes.length){
          ...
          map.addLayer({id:'afat-transit-nodes',type:'circle',source:'afat-transit-nodes',paint:{'circle-radius':['interpolate',['linear'],['zoom'],10,3,15,7] as any,'circle-color':'#22d3ee','circle-stroke-color':'#082f49','circle-stroke-width':2,'circle-opacity':0.96}});
        }
      }
  ...
  const chooseMode=(mode:'truth'|'uncertainty'|'evidence')=>{
    ...
    if(mode==='uncertainty') setLayers({provisional:true,corroborated:false,verified:false,candidates:true,observations:false,reachability:true,transit:false});
