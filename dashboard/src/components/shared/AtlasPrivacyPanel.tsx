...
export function AtlasPrivacyPanel(){
 ...
 return <section className="rounded-[1.5rem] border border-blue-300/15 bg-blue-400/[0.04] p-5">
   <div className="flex items-start gap-3"><ShieldCheck className="h-5 w-5 text-blue-200"/><div><p className="text-[10px] font-black uppercase tracking-widest text-blue-200">{t('atlas.privacy')}</p><h2 className="mt-1 text-lg font-black">{t('atlas.privacyTitle')}</h2><p className="mt-1 text-xs leading-5 text-white/45">Raw GPS can expire while sanitized aggregate evidence remains useful. Turning aggregate learning off prevents future movement observations and candidate geometry from entering the Atlas.</p></div></div>
   ...
   <button onClick={save} disabled={busy} className="mt-4 min-h-11 w-full rounded-xl bg-blue-300 text-[10px] font-black uppercase text-slate-950 disabled:opacity-35">{t('atlas.savePrivacy')}</button>
