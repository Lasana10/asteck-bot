...
import { Crosshair, LocateFixed, MapPin, Navigation2 } from 'lucide-react';
...
function CameraGuide({ origin, destination }: { origin: SpatialPoint | null; destination: SpatialPoint | null }) {
  const map = useMap();
  useEffect(() => {
  ...
  }, [map, origin?.latitude, origin?.longitude, destination?.latitude, destination?.longitude]);
  return null;
}

export function PassengerSpatialMap({ destination, meetingPoint, city = 'yaounde', onOriginResolved }: Props) {
  ...
  return <section className="overflow-hidden rounded-[28px] border border-white/10 bg-slate-950/80 shadow-[0_24px_70px_rgba(0,0,0,0.32)]">
    <div className="relative h-[280px] sm:h-[340px]">
      <MapContainer center={YAOUNDE_CENTER} zoom={13} zoomControl={false} scrollWheelZoom className="absolute inset-0 h-full w-full">
        ...
        <CameraGuide origin={origin} destination={arrivalPoint || null} />
      ...
      <div className="absolute left-4 top-4 z-[600] max-w-[72%] rounded-2xl border border-white/10 bg-slate-950/72 px-4 py-3 backdrop-blur-xl">
        <div className="flex items-center gap-2"><Navigation2 className="h-4 w-4 text-blue-300" /><p className="text-[9px] font-black uppercase tracking-[0.18em] text-white/45">{hasArrival ? 'Your arrival' : 'Explore nearby'}</p></div>
        ...
        {meetingPoint?.instructions && <p className="mt-1 line-clamp-2 text-[10px] leading-4 text-white/55">{meetingPoint.instructions}</p>}
      </div>
    ...
    {locationMessage && <div className="border-t border-white/8 px-4 py-3 text-[10px] font-semibold text-white/50">{locationMessage}</div>}
  </section>;
}
