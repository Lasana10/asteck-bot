        ...
        <Surface className="overflow-hidden bg-gradient-to-br from-blue-500/[0.14] via-cyan-400/[0.045] to-transparent p-6 sm:p-8">
          ...
          <form onSubmit={submit} className="mt-7 space-y-3">
            <label className="flex min-h-14 items-center gap-3 rounded-xl border border-white/10 bg-black/25 px-4">
              ...
              <input value={origin} onChange={(event) => setOrigin(event.target.value)} className="w-full bg-transparent text-sm font-bold outline-none" />
            </label>
            <label className="flex min-h-14 items-center gap-3 rounded-xl border border-white/10 bg-black/25 px-4">
              <Search className="h-4 w-4 text-blue-300" />
              <input value={destination} onChange={(event) => setDestination(event.target.value)} placeholder="Destination or landmark" className="w-full bg-transparent text-sm font-bold outline-none placeholder:text-white/25" />
            </label>
