...
export function OperatorWorkspace({ profile, live, missions, currentDispatch, onNavigate, onChanged }: Props) {
  ...
  const accept = async () => {
    ...
    setBusy(true);
    const { error } = await updatePassageIntentStatus(mission.id, {
  ...
  const activeMission = Boolean(currentDispatch && !['completed','cancelled','expired','declined','no_show'].includes(String(currentDispatch.status || '').toLowerCase()));

  return <div className="grid gap-5 xl:grid-cols-[0.74fr_1.26fr]">
    <div className="space-y-5">
      ...
      <Surface className="p-5">
        ...
        <p className="mt-2 text-sm text-white/45">
          {mission ? `${mission.origin_text || 'Origin pending'} → ${mission.destination_text || 'Destination pending'}` : 'AFAT will show only a real eligible request.'}
        </p>
        ...
        <div className="mt-5 flex gap-2">
          <button onClick={accept} disabled={!vehicle?.is_available || !mission?.id || busy || activeMission} className="min-h-12 flex-1 rounded-xl bg-emerald-400 px-4 text-xs font-black text-slate-950 disabled:opacity-35">
            Accept request
          </button>
    ...
    <div className="min-h-[620px]"><MapPanel role="operator" live={live} /></div>
  </div>;
}
